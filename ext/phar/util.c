...
static phar_archive_data *phar_copy_cached_phar(phar_archive_data *sphar) /* {{{ */
{
	phar_archive_data *phar;
	...
	*phar = *sphar;
	...
	zend_hash_copy(&newmanifest, &(sphar->manifest), phar_manifest_copy_ctor);
	...
	zend_hash_init(&phar->virtual_dirs, sizeof(char *),
		zend_get_hash_value, NULL, 0);
	zend_hash_copy(&phar->virtual_dirs, &(sphar->virtual_dirs), NULL);
...
int phar_copy_on_write(phar_archive_data **pphar) /* {{{ */
{
	...
	newpphar = phar_copy_cached_phar((phar_archive_data *)Z_PTR_P(pzv));
	/* invalidate phar cache */
	PHAR_G(last_phar) = NULL;
	PHAR_G(last_phar_name) = PHAR_G(last_alias) = NULL;
